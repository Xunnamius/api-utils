// @ts-check

import {
  assertEnvironment,
  moduleExport
} from '@-xun/symbiote/assets/eslint.config.mjs';

import { createDebugLogger } from 'rejoinder';

const debug = createDebugLogger({ namespace: 'symbiote:config:eslint' });

const config = await moduleExport({
  derivedAliases: getEslintAliases(),
  ...(await assertEnvironment())
});

/* Add custom config objects here, such as those disabling certain rules */
config
  .push
  // ? Paths listed here are ignored by Eslint and related tooling.
  // { ignores: [] },
  // * Configs applying to both JavaScript & TypeScript files (all extensions)
  // ? Keep in mind that JS files can use @ts-check and "become" TS files,
  // ? hence the existence of this block. Logically, most rules should be
  // ? loaded here.
  // ...[
  //   { ...eslintPluginReactConfigs.flat.recommended, name: 'react:recommended' },
  //   // ? For react@>=17
  //   { ...eslintPluginReactConfigs.flat['jsx-runtime'], name: 'react:jsx-runtime' },
  //   {
  //     ...eslintPluginReactHooksConfigs['recommended-latest'],
  //     name: 'react-hooks:recommended-latest'
  //   },
  //   { ...eslintPluginJsxA11yFlatConfigs.recommended, name: 'jsx-a11y:recommended' }
  // ].flatMap((configs) =>
  //   overwriteProperty(configs, 'files', [
  //     `**/*.{ts,cts,mts,tsx}`
  //   ])
  // ),
  // {
  //   rules: {
  //     'unicorn/no-keyword-prefix': 'off',
  //     'no-restricted-syntax': 'off'
  //   }
  // }
  ();

export default config;

debug('exported config: %O', config);

function getEslintAliases() {
  // ! These aliases are auto-generated by symbiote. Instead of modifying them
  // ! directly, consider regenerating aliases across the entire project with:
  // ! `npx symbiote project renovate --regenerate-assets --assets-preset ...`
  return [
    ['multiverse+api:*', './packages/api/src/*'],
    ['multiverse+api-error:*', './packages/api-error/src/*'],
    ['multiverse+api-strategy:*', './packages/api-strategy/src/*'],
    ['multiverse+env:*', './packages/env/src/*'],
    ['multiverse+respond:*', './packages/respond/src/*'],
    ['multiverse+shared:*', './packages/shared/src/*'],
    ['multiverse+api', './packages/api/src/index.ts'],
    ['multiverse+api-error', './packages/api-error/src/index.ts'],
    ['multiverse+api-strategy', './packages/api-strategy/src/index.ts'],
    ['multiverse+env', './packages/env/src/index.ts'],
    ['multiverse+respond', './packages/respond/src/index.ts'],
    ['multiverse+shared', './packages/shared/src/index.ts'],
    ['rootverse+api:*', './packages/api/*'],
    ['rootverse+api-error:*', './packages/api-error/*'],
    ['rootverse+api-strategy:*', './packages/api-strategy/*'],
    ['rootverse+env:*', './packages/env/*'],
    ['rootverse+respond:*', './packages/respond/*'],
    ['rootverse+shared:*', './packages/shared/*'],
    ['rootverse:*', './*'],
    ['universe+api:*', './packages/api/src/*'],
    ['universe+api-error:*', './packages/api-error/src/*'],
    ['universe+api-strategy:*', './packages/api-strategy/src/*'],
    ['universe+env:*', './packages/env/src/*'],
    ['universe+respond:*', './packages/respond/src/*'],
    ['universe+shared:*', './packages/shared/src/*'],
    ['universe+api', './packages/api/src/index.ts'],
    ['universe+api-error', './packages/api-error/src/index.ts'],
    ['universe+api-strategy', './packages/api-strategy/src/index.ts'],
    ['universe+env', './packages/env/src/index.ts'],
    ['universe+respond', './packages/respond/src/index.ts'],
    ['universe+shared', './packages/shared/src/index.ts'],
    ['universe:*', './src/*'],
    ['universe', './src/index.ts'],
    ['testverse+api:*', './packages/api/test/*'],
    ['testverse+api-error:*', './packages/api-error/test/*'],
    ['testverse+api-strategy:*', './packages/api-strategy/test/*'],
    ['testverse+env:*', './packages/env/test/*'],
    ['testverse+respond:*', './packages/respond/test/*'],
    ['testverse+shared:*', './packages/shared/test/*'],
    ['testverse:*', './test/*'],
    ['typeverse:*', './types/*']
  ];
}
