...
import {
  ...
  name as packageName
} from 'rootverse+api:package.json';
...
const debug = createDebugLogger({ namespace: 'api' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+api:package.json'))),
    packageName,
    packageExports
  );
});
