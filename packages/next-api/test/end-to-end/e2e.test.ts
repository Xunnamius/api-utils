...
import {
  ...
  name as packageName
} from 'rootverse+next-api:package.json';
...
const debug = createDebugLogger({ namespace: 'next-api' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+next-api:package.json'))),
    packageName,
    packageExports
  );
});
