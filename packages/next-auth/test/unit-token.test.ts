...
describe('::deleteTokenById', () => {
  it('deletes an auth entry by auth_id ans returns deleted count', async () => {
    ...
    await expect(deleteTokenById({ auth_id: dummyRootData.auth[1]._id })).resolves.toBe(
      1
    );

    await expect(countAuthDbTokens()).resolves.toBe(dummyRootData.auth.length - 1);

    await expect(deleteTokenById({ auth_id: dummyRootData.auth[0]._id })).resolves.toBe(
      1
    );
  ...
  it('rejects if passed invalid data', async () => {
    ...
    const errors: [params: Parameters<typeof deleteTokenById>[0], error: string][] = [
      ...
      [{ auth_id: false as unknown as ObjectId }, ErrorMessage.InvalidObjectId(false)],
...
describe('::getTokensByAttribute', () => {
  ...
  it('returns empty array if no matching attributes exist', async () => {
    ...
    await expect(getTokensByAttribute({ filter: { owner: [] } })).resolves.toStrictEqual(
      []
    );
  });
...
describe('::isTokenAttributesFilter', () => {
  it('returns true only if passed TokenAttributesFilter', async () => {
    ...
    expect(isTokenAttributesFilter({ owner: 'owner', isGlobalAdmin: false })).toBeTrue();
...
describe('::updateTokenAttributesById', () => {
  ...
  it('rejects if passed invalid data', async () => {
    ...
    const errors: [
      ...
      [{ auth_id, update: { isGlobalAdmin: 5 } }, ErrorMessage.InvalidSecret('update')],
      [{ auth_id, update: { owners: ['prop'] } }, ErrorMessage.InvalidSecret('update')],
      [
...
describe('::updateTokensAttributesByAttribute', () => {
  ...
  it('updates (patches) multiple existing auth entries', async () => {
    ...
    await expect(
      updateTokensAttributesByAttribute({
        filter: {
          owner: dummyRootData.auth.slice(0, -1).map((entry) => entry.attributes.owner)
        },
        update: { owner: 'xyz123' }
      })
  ...
  it('rejects if passed invalid data', async () => {
    ...
    const errors: [
      ...
      [{ filter: { name: 'owner' }, update: {} }, ErrorMessage.InvalidSecret('filter')],
      [{ filter: {}, update: { name: 'owner' } }, ErrorMessage.InvalidSecret('update')],
