...
import {
  ...
  name as packageName
} from 'rootverse+respond:package.json';
...
const debug = createDebugLogger({ namespace: 'respond' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+respond:package.json'))),
    packageName,
    packageExports
  );
});
