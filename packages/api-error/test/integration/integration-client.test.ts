...
import {
  ...
  name as packageName
} from 'rootverse+api-error:package.json';
...
const debug = createDebugLogger({ namespace: 'api-error' }).extend(TEST_IDENTIFIER);
...
beforeAll(async () => {
  await ensurePackageHasBeenBuilt(
    toDirname(toAbsolutePath(require.resolve('rootverse+api-error:package.json'))),
    packageName,
    packageExports
  );
});
